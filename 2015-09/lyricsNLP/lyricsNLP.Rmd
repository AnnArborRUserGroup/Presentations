...
collection <- read_lines("lyrics.txt")
...
Then, create functions to access musiXmatch API:
  ...
  - get all album IDs
  - using artist ID and track IDs, get track IDs
...
## k-means clustering
  ...
  - classify __n__ documents into __k__ clusters  
...
major R packages: ```tm```, ```topicmodels```, ```slam```, ```wordcloud```  
