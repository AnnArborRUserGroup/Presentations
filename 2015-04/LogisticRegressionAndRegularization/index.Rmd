...
x <- seq(-8, 8, by=.1)
gx <- g(x)
qplot(x, gx, geom="line", xlab="z", ylab="g(z)")
...
library(reshape2)
x <- seq(0, 1, by=.01)
h1 <- function(x) return(-log(x))
h0 <- function(x) return(-log(1-x))
h1x <- h1(x)
h0x <- h0(x)
dt <- data.frame(x, h1x, h0x)
dtmelt <- melt(dt, id.vars="x", variable.name="h")
dtmelt$Case <- ifelse(dtmelt$h=="h1x", "When y=1", "When y=0")
ggplot(dtmelt, aes(x, value, color=Case)) +
  geom_line() + xlab("Prediction") + ylab("Cost")
...
```{r, echo=FALSE}
# Build a simple logistic regression for comparison using glm()
mdl <- glm(Salary~., data=Hitters[train, ], family="binomial")
mdl.preds <- round(predict(mdl, Hitters[test, ], type="response"))
mdl.conf <- table(mdl.preds, Hitters$Salary[test])
```
...
```{r, echo=FALSE}
ridge.conf <- table(round(ridge.pred), y[test])
```
for an accuracy of `r sum(diag(ridge.conf))/sum(ridge.conf)`.  Running a plain-old logistic regression using *glm()* gives us an accuracy of `r sum(diag(mdl.conf))/sum(mdl.conf)`, and so the ridge regularization provides a moderate improvement in accuracy.
...
```{r, echo=FALSE}
lassomod.conf <- table(factor(new.class), y[test])
```
Which gives us an accuracy of `r sum(diag(lassomod.conf))/sum(lassomod.conf)`, which out-performs both the performance of the original logistic regression and the ridge regression (with accuracies of `r sum(diag(mdl.conf))/sum(mdl.conf)` and `r sum(diag(ridge.conf))/sum(ridge.conf)`, respectively).
...
```{r, echo=FALSE}
lbest_lambda <- cv.lassomod$lambda.min
best.lasso <- glmnet(x[train, ], y[train], lambda=lbest_lambda, 
                family="binomial", alpha=1)
best.lasso$beta
```
